...
class StoreEdit extends React.Component{
    ...
    addSelectedImages(){
        ...
        axios.put(server + '/mob/stores/'+this.state.id, formdata).then(resp=>{
    ...
    removeImage(id){
        if(window.confirm('Bozmaga ynamyňyz barmy?')===true)
            axios.post(server + '/mob/stores/img/delete/'+ id ).then(resp=>{
                this.setData()
            }).catch(err=>{
                alert('Ýalňyşlyk ýüze çykdy')
            });
    ...
    deleteStore(id){
        ...
        axios.post(server + '/mob/stores/delete/'+ id ).then(resp=>{
                this.setData()
                window.history.back()
            }).catch(err=>{
                alert('Ýalňyşlyk ýüze çykdy')
    ...
    save(){
        ...
        if(document.getElementById('delivery').checked){formdata.append('has_delivery', true)}
        else{formdata.append('has_delivery', false)}
                       
        formdata.append('category', document.getElementById('category').value)       
        formdata.append('center', document.getElementById('trade_center').value)
        formdata.append('body_tm', document.getElementById('body_tm').value)
        ...
        axios.put(server + '/mob/stores/'+this.state.id , formdata).then(resp=>{
            alert('Ýatda saklandy');
            this.setData();
        }).catch(err=>{alert('Ýalňyşlyk ýüze çykdy')});
    ...
    addPhone(){
        ...
        axios.put(server + '/mob/stores/'+ this.state.id, fdata).then(resp=>{
            this.setData();
        });
    ...
    deletePhone(phone){
        ...
        let result = window.confirm( name + ' '+phone.phone + ' - telefon belgini bozmaga ynamyňyz barmy')
        if(result === true){
            axios.post(server + '/mob/stores/phone/delete/'+ phone.id).then(resp=>{
              this.setData();
            });
        }
    }
    ...
    render(){
        ...
        return <div className="stores_edit p-[10px] text-[14px]">

            <div className="preview flex m-[10px]">
                <img alt="" className="rounded-lg shadow-md  object-cover w-[100px] h-[100px]" src={main_img}></img>
                <div className="grid h-max mx-[10px]">
                    <label className="sm:text-[17px] font-bold">{this.state.name}</label>
                    <label className="flex items-center"> <BiMap></BiMap> {this.state.location.name}</label>
                    <div className="flex items-center sm:text-[12px]">
                        <label className="my-[10px] flex items-center mr-[10px]"> <FiEye></FiEye>  {this.state.viewed}  </label>
                        <label className="my-[10px] flex items-center mr-[10px]"> <BiTime></BiTime> {this.state.created_at}</label>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-1">
                ...
                <div className="">
                    ...
                    <div className="flex flex-wrap">
                        {
                            this.state.images.map(item=>{
                                ...
                                return <div className="relative m-[5px]" key={item.id}>
                                    <img alt="" className="h-[100px] w-[100px] object-cover rounded-md" src={img}></img>
                    ...
                    <div>
                        <button className="p-[5px] text-white  bg-sky-600" 
                        onClick={()=>{document.getElementById('imgselector').click()}}>Surat goş</button>
                    </div>
                ...
                <div className="inputFields grid max-w-[400px] text-[13px]">      
                        ...
                        <input className="field hover:border-sky-500 border" id="name" 
                        ...
                        <label className="field_name">Ýerleşýän söwda merkezi</label>
                        <select className="field" id="trade_center">
                            <option value={''}>{this.state.trade_center}</option>
                        ...
                        <div className="flex items-center border rounded-md p-[10px] m-[5px]">
                            {this.state.delivery === 'True' && <input defaultChecked id="delivery" type="checkbox"></input>}
                            {this.state.delivery === 'False' && <input id="delivery" type="checkbox"></input>}
                            <label>Eltip bermek hyzmaty</label>
                        </div>

                        <label className="field_name">Giňişleýin maglumat</label>
                        <textarea className="field" id="body_tm" defaultValue={this.state.detail_text}></textarea>

                        <div className="flex">
                            <input placeholder="Telefon belgisi " className="field" id="new_contact"></input>
                            <button 
                        ...
                        <div className="phone_list">
                        {
                            this.state.phones.map(item=>{
                                return <div className="border p-[5px] m-[5px] w-max rounded">
                                    <BiPhone size={20}></BiPhone>
                                    <label>{item.phone} {item.name}  </label>
                                    <MdClose size={25} 
                                        onClick={()=>{this.deletePhone(item)}} c
                                        lassName="close duration-300"></MdClose>
                                </div>
                            })
                        }
                    </div>
                    ...
                    <div className="flex">
                        <button onClick={()=>{this.save()}} 
                            className="bg-green-700 text-white w-max p-[10px]">Ýatda sakla</button>
                        <button onClick={()=>{this.deleteStore(this.state.id)}} 
                            className="bg-red-700 text-white w-max p-[10px]">Bozmak</button>
